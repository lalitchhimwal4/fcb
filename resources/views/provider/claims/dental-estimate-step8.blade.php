...
<section class="main-page-wrap">
    <div class="family-profile-sec">
        ...
        <div class="enroll-content-outer provider-dashboard edit-provider-details">
            <form action="{{route('provider.submit_claim_step8')}}" method="post">
                <div class="container">
                    <div class="enroll-content-wrap">
                        @csrf
                        @include('showmessages')
                        <div>
                            <h4>Treatment Estimate Submission</h4>
                        ...
                        <h5 class="claim-form-heading"><span></span> Summary & Final Approval</h5>
                        <div class="form d-flex flex-wrap step8-claim-form">
                            <div class="form-group">
                                <table class="table claim-submission-table">
                                    <thead>
                                        <tr>
                                            <th>Estimate Summary</th>
                                            <th></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <td>Total fee guide amount</td>
                                            <td>-</td>
                                        </tr>
                                        <tr>
                                            <td>Total FCB contracted rate</td>
                                            <td>${{number_format((float)$validated['claim']['totalServiceChargeAmount'], 2, '.', '') ?? ''}}</td>
                                        </tr>
                                        <tr>
                                            <td>Total eligible lab fee</td>
                                            <td>-</td>
                                        </tr>
                                        <tr>
                                            <td>Total eligible expense fee</td>
                                            <td>-</td>
                                        </tr>
                                        <tr>
                                            <td>Total submitted amount</td>
                                            <td>${{number_format((float)$validated['claim']['totalSubmittedAmount'], 2, '.', '') ?? '-'}}</td>
                                        </tr>
                                        <tr>
                                            <td>Total benefit relief</td>
                                            <td>${{number_format((float)$validated['claim']['totalPlanPaysAmount'], 2, '.', '') ?? '-'}}</td>
                                        </tr>
                                        <tr>
                                            <td>Patient responsibility</td>
                                            <td>${{number_format((float)$validated['claim']['totalPatientPaysAmount'], 2, '.', '') ?? '-'}}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <table class="table claim-step-8-table">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>{{$validated['claim']['claimCode'] ?? '-'}}</td>
                                        </tr>
                                        <tr>
                                            <th>Date Processed</th>
                                            <td>{{Carbon::create($validated['claim']['processDate'])->toDateString() ?? '-'}}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
